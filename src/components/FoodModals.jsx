    ...
    try {
      ...
      const logPromises = foodsToAdd.map((food, idx) => {
        ...
        return apiPost('/.netlify/functions/food-diary', {
          ...
          vitaminC: food.vitaminC != null ? Math.round((food.vitaminC * foodServings) * 10) / 10 : null,
...
export function SearchFoodsModal({ isOpen, onClose, mealType, clientData, onFoodLogged }) {
  ...
  const getScaledNutrition = () => {
    ...
    return {
      ...
      fiber: selectedFood.fiber != null ? Math.round((selectedFood.fiber * multiplier) * 10) / 10 : null,
      sugar: selectedFood.sugar != null ? Math.round((selectedFood.sugar * multiplier) * 10) / 10 : null,
      sodium: selectedFood.sodium != null ? Math.round(selectedFood.sodium * multiplier) : null,
      potassium: selectedFood.potassium != null ? Math.round(selectedFood.potassium * multiplier) : null,
      calcium: selectedFood.calcium != null ? Math.round(selectedFood.calcium * multiplier) : null,
      iron: selectedFood.iron != null ? Math.round((selectedFood.iron * multiplier) * 10) / 10 : null,
      vitaminC: selectedFood.vitaminC != null ? Math.round((selectedFood.vitaminC * multiplier) * 10) / 10 : null,
      cholesterol: selectedFood.cholesterol != null ? Math.round(selectedFood.cholesterol * multiplier) : null
    };
  };
...
export function ScanLabelModal({ isOpen, onClose, mealType, clientData, onFoodLogged }) {
  ...
  const getScaledNutrition = () => {
    ...
    return {
      ...
      fiber: result.fiber != null ? Math.round((result.fiber * servings) * 10) / 10 : null,
      sugar: result.sugar != null ? Math.round((result.sugar * servings) * 10) / 10 : null,
      sodium: result.sodium != null ? Math.round(result.sodium * servings) : null,
      potassium: result.potassium != null ? Math.round(result.potassium * servings) : null,
      calcium: result.calcium != null ? Math.round(result.calcium * servings) : null,
      iron: result.iron != null ? Math.round((result.iron * servings) * 10) / 10 : null,
      vitaminC: result.vitaminC != null ? Math.round((result.vitaminC * servings) * 10) / 10 : null,
      cholesterol: result.cholesterol != null ? Math.round(result.cholesterol * servings) : null
    };
  };
