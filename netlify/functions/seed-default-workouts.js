...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Check if this coach already has ANY workout programs (not just defaults)
    ...
    // Skip if coach already has programs
    if (existingCount > 0) {
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ success: true, message: 'Coach already has programs', seeded: false })
      };
    }

    // ── Enrich exercises with DB data (video, thumbnail, etc.) ──────────
    const allExerciseNames = [...new Set(
      DEFAULT_PROGRAMS.flatMap(prog =>
